<!--page title start-->

<section class="page-title parallaxie" data-bg-img="images/client/img_7.jpg">
  <div class="container">
    <div class="row">
      <div class="col-lg-6">
        <div class="white-bg p-md-5 p-3 d-inline-block">
        <h1 class="text-theme">Irrigation Water Quality <span class="text-black">Testing Packages</span></h1>
        <nav aria-label="breadcrumb" class="page-breadcrumb border-top border-light pt-3 mt-3">
          <ol class="breadcrumb">
            <li class="breadcrumb-item"><a href="<?php echo base_url('/') ?>"><i class="las la-home me-1"></i>Home</a>
            </li>
            <li class="breadcrumb-item">Research
            </li>
            <li class="breadcrumb-item active" aria-current="page">Irrigation Water Quality Testing  Packages</li>
          </ol>
        </nav>
        </div>
      </div>
    </div>
  </div>
</section>

<!--page title end-->


<!--body content start-->

<div class="page-content">


<!--project start-->

<section class="pt-0">
  <div class="container">
    <div class="row justify-content-center text-center">
      <div class="col-lg-8">
        <div class="section-title"><br><br><br>
          <h2 class="title">Irrigation Water Quality Testing Packages 2024</h2><br><br>

    
          <table border="1" cellpadding="8" cellspacing="0" style="border-collapse: collapse; width: 100%;">
<tr style="background-color: #f2f2f2;">
<th rowspan="2" style="border: 1px solid black; padding: 10px;">No.</th>
<th rowspan="2" style="border: 1px solid black; padding: 10px;">Parameters</th>
<th rowspan="2" style="border: 1px solid black; padding: 10px;">Analysis Method</th>
<th rowspan="2" style="border: 1px solid black; padding: 10px;">Cost per Sample</th>
<th rowspan="2" style="border: 1px solid black; padding: 10px;">Remarks</th>
</tr>
<tr></tr>
<!-- Basic Water Quality Parameters -->
<tr>
<td style="text-align: center; border: 1px solid black;">1</td>
<td style="border: 1px solid black;">Temperature</td>
<td style="border: 1px solid black;">Electrode</td>
<td style="text-align: right; border: 1px solid black;">$2.00</td>
<td style="border: 1px solid black;"></td>
</tr>
<tr>
<td style="text-align: center; border: 1px solid black;">2</td>
<td style="border: 1px solid black;">Dissolved Oxygen (DO)</td>
<td style="border: 1px solid black;">Membrane Electrode</td>
<td style="text-align: right; border: 1px solid black;">$9.00</td>
<td style="border: 1px solid black;"></td>
</tr>
<tr>
<td style="text-align: center; border: 1px solid black;">3</td>
<td style="border: 1px solid black;">pH</td>
<td style="border: 1px solid black;">Electrometric</td>
<td style="text-align: right; border: 1px solid black;">$3.00</td>
<td style="border: 1px solid black;"></td>
</tr>
<tr>
<td style="text-align: center; border: 1px solid black;">4</td>
<td style="border: 1px solid black;">ORP</td>
<td style="border: 1px solid black;">Electrode</td>
<td style="text-align: right; border: 1px solid black;">$5.00</td>
<td style="border: 1px solid black;"></td>
</tr>
<tr>
<td style="text-align: center; border: 1px solid black;">5</td>
<td style="border: 1px solid black;">Electrical Conductivity (EC)</td>
<td style="border: 1px solid black;">Electrode</td>
<td style="text-align: right; border: 1px solid black;">$3.00</td>
<td style="border: 1px solid black;"></td>
</tr>
<tr>
<td style="text-align: center; border: 1px solid black;">6</td>
<td style="border: 1px solid black;">Sodium (Na)</td>
<td style="border: 1px solid black;">Electrode</td>
<td style="text-align: right; border: 1px solid black;">$5.00</td>
<td style="border: 1px solid black;"></td>
</tr>
<tr>
<td style="text-align: center; border: 1px solid black;">7</td>
<td style="border: 1px solid black;">Potassium (K)</td>
<td style="border: 1px solid black;">Spectrophotometry</td>
<td style="text-align: right; border: 1px solid black;">$9.00</td>
<td style="border: 1px solid black;"></td>
</tr>
<tr>
<td style="text-align: center; border: 1px solid black;">8</td>
<td style="border: 1px solid black;">Magnesium (Mg)</td>
<td style="border: 1px solid black;">Spectrophotometry</td>
<td style="text-align: right; border: 1px solid black;">$7.00</td>
<td style="border: 1px solid black;"></td>
</tr>
<tr>
<td style="text-align: center; border: 1px solid black;">9</td>
<td style="border: 1px solid black;">Calcium (Ca)</td>
<td style="border: 1px solid black;">Titration</td>
<td style="text-align: right; border: 1px solid black;">$5.00</td>
<td style="border: 1px solid black;"></td>
</tr>
<tr>
<td style="text-align: center; border: 1px solid black;">10</td>
<td style="border: 1px solid black;">Iron (Fe)</td>
<td style="border: 1px solid black;">Spectrophotometry</td>
<td style="text-align: right; border: 1px solid black;">$5.00</td>
<td style="border: 1px solid black;"></td>
</tr>
<tr>
<td style="text-align: center; border: 1px solid black;">11</td>
<td style="border: 1px solid black;">Manganese (Mn)</td>
<td style="border: 1px solid black;">Spectrophotometry</td>
<td style="text-align: right; border: 1px solid black;">$5.00</td>
<td style="border: 1px solid black;"></td>
</tr>
<tr>
<td style="text-align: center; border: 1px solid black;">12</td>
<td style="border: 1px solid black;">Chloride (Cl-)</td>
<td style="border: 1px solid black;">Spectrophotometry</td>
<td style="text-align: right; border: 1px solid black;">$7.00</td>
<td style="border: 1px solid black;"></td>
</tr>
<tr>
<td style="text-align: center; border: 1px solid black;">13</td>
<td style="border: 1px solid black;">Sulfate (SO<sub>4</sub><sup>2-</sup>)</td>
<td style="border: 1px solid black;">Spectrophotometry</td>
<td style="text-align: right; border: 1px solid black;">$5.00</td>
<td style="border: 1px solid black;"></td>
</tr>
<tr>
<td style="text-align: center; border: 1px solid black;">14</td>
<td style="border: 1px solid black;">Boron (B)</td>
<td style="border: 1px solid black;">Spectrophotometry</td>
<td style="text-align: right; border: 1px solid black;">$10.00</td>
<td style="border: 1px solid black;"></td>
</tr>
<tr>
<td style="text-align: center; border: 1px solid black;">15</td>
<td style="border: 1px solid black;">Nitrate (NO<sub>3</sub><sup>-</sup>)</td>
<td style="border: 1px solid black;">Spectrophotometry</td>
<td style="text-align: right; border: 1px solid black;">$7.00</td>
<td style="border: 1px solid black;"></td>
</tr>
<tr>
<td style="text-align: center; border: 1px solid black;">16</td>
<td style="border: 1px solid black;">Nitrite (NO<sub>2</sub><sup>-</sup>)</td>
<td style="border: 1px solid black;">Spectrophotometry</td>
<td style="text-align: right; border: 1px solid black;">$9.00</td>
<td style="border: 1px solid black;"></td>
</tr>
<tr>
<td style="text-align: center; border: 1px solid black;">17</td>
<td style="border: 1px solid black;">Ammonia (NH<sub>3</sub>)</td>
<td style="border: 1px solid black;">Spectrophotometry</td>
<td style="text-align: right; border: 1px solid black;">$5.00</td>
<td style="border: 1px solid black;"></td>
</tr>
<tr>
<td style="text-align: center; border: 1px solid black;">18</td>
<td style="border: 1px solid black;">Ammonium (NH<sub>4</sub><sup>+</sup>)</td>
<td style="border: 1px solid black;">Spectrophotometry</td>
<td style="text-align: right; border: 1px solid black;">$7.00</td>
<td style="border: 1px solid black;"></td>
</tr>
<tr>
<td style="text-align: center; border: 1px solid black;">19</td>
<td style="border: 1px solid black;">Total Nitrogen (TN)</td>
<td style="border: 1px solid black;">Spectrophotometry</td>
<td style="text-align: right; border: 1px solid black;">$15.00</td>
<td style="border: 1px solid black;"></td>
</tr>
<tr>
<td style="text-align: center; border: 1px solid black;">20</td>
<td style="border: 1px solid black;">Sulfide (S<sub>2</sub><sup>-</sup>)</td>
<td style="border: 1px solid black;">Spectrophotometry</td>
<td style="text-align: right; border: 1px solid black;">$5.00</td>
<td style="border: 1px solid black;"></td>
</tr>
<tr>
<td style="text-align: center; border: 1px solid black;">21</td>
<td style="border: 1px solid black;">Phosphate (PO<sub>4</sub><sup>3-</sup>)</td>
<td style="border: 1px solid black;">Spectrophotometry</td>
<td style="text-align: right; border: 1px solid black;">$7.00</td>
<td style="border: 1px solid black;"></td>
</tr>
<tr>
<td style="text-align: center; border: 1px solid black;">22</td>
<td style="border: 1px solid black;">Silica (SiO<sub>2</sub>)</td>
<td style="border: 1px solid black;">Spectrophotometry</td>
<td style="text-align: right; border: 1px solid black;">$9.00</td>
<td style="border: 1px solid black;"></td>
</tr>
<tr>
<td style="text-align: center; border: 1px solid black;">23</td>
<td style="border: 1px solid black;">Total Phosphorus (TP)</td>
<td style="border: 1px solid black;">Spectrophotometry</td>
<td style="text-align: right; border: 1px solid black;">$12.00</td>
<td style="border: 1px solid black;"></td>
</tr>
<tr>
<td style="text-align: center; border: 1px solid black;">24</td>
<td style="border: 1px solid black;">Hardness, Total</td>
<td style="border: 1px solid black;">Titration</td>
<td style="text-align: right; border: 1px solid black;">$5.00</td>
<td style="border: 1px solid black;"></td>
</tr>
<tr style="background-color: #f2f2f2;">
<th colspan="5" style="border: 1px solid black; text-align: center; padding: 10px;">Toxic Heavy Metals</th>
</tr>
<tr>
<td style="text-align: center; border: 1px solid black;">25</td>
<td style="border: 1px solid black;">Arsenic (As)</td>
<td style="border: 1px solid black;">Electrode</td>
<td style="text-align: right; border: 1px solid black;">$15.00</td>
<td style="border: 1px solid black;"></td>
</tr>
<tr>
<td style="text-align: center; border: 1px solid black;">26</td>
<td style="border: 1px solid black;">Barium (Ba)</td>
<td style="border: 1px solid black;">Spectrophotometry</td>
<td style="text-align: right; border: 1px solid black;">$10.00</td>
<td style="border: 1px solid black;"></td>
</tr>
<tr>
<td style="text-align: center; border: 1px solid black;">27</td>
<td style="border: 1px solid black;">Cadmium (Cd)</td>
<td style="border: 1px solid black;">Spectrophotometry</td>
<td style="text-align: right; border: 1px solid black;">$20.00</td>
<td style="border: 1px solid black;"></td>
</tr>
<tr>
<td style="text-align: center; border: 1px solid black;">28</td>
<td style="border: 1px solid black;">Chromium (Cr)</td>
<td style="border: 1px solid black;">Spectrophotometry</td>
<td style="text-align: right; border: 1px solid black;">$10.00</td>
<td style="border: 1px solid black;"></td>
</tr>
<tr>
<td style="text-align: center; border: 1px solid black;">29</td>
<td style="border: 1px solid black;">Lead (Pb)</td>
<td style="border: 1px solid black;">Spectrophotometry</td>
<td style="text-align: right; border: 1px solid black;">$20.00</td>
<td style="border: 1px solid black;"></td>
</tr>
<tr>
<td style="text-align: center; border: 1px solid black;">30</td>
<td style="border: 1px solid black;">Nickel (Ni)</td>
<td style="border: 1px solid black;">Spectrophotometry</td>
<td style="text-align: right; border: 1px solid black;">$15.00</td>
<td style="border: 1px solid black;"></td>
</tr>
<tr>
<td style="text-align: center; border: 1px solid black;">31</td>
<td style="border: 1px solid black;">Copper (Cu)</td>
<td style="border: 1px solid black;">Spectrophotometry</td>
<td style="text-align: right; border: 1px solid black;">$10.00</td>
<td style="border: 1px solid black;"></td>
</tr>
<tr>
<td style="text-align: center; border: 1px solid black;">32</td>
<td style="border: 1px solid black;">Zinc (Zn)</td>
<td style="border: 1px solid black;">Spectrophotometry</td>
<td style="text-align: right; border: 1px solid black;">$15.00</td>
<td style="border: 1px solid black;"></td>
</tr>
<tr>
<td style="text-align: center; border: 1px solid black;">33</td>
<td style="border: 1px solid black;">Cobalt (Co)</td>
<td style="border: 1px solid black;">Spectrophotometry</td>
<td style="text-align: right; border: 1px solid black;">$15.00</td>
<td style="border: 1px solid black;"></td>
</tr>
<tr>
<td style="text-align: center; border: 1px solid black;">34</td>
<td style="border: 1px solid black;">Molybdenum (Mo)</td>
<td style="border: 1px solid black;">Spectrophotometry</td>
<td style="text-align: right; border: 1px solid black;">$15.00</td>
<td style="border: 1px solid black;"></td>
</tr>
</table>

        </div>
      </div>
    </div>
  </div>
</section>

<!--project end-->

</div>

<!--body content end-->