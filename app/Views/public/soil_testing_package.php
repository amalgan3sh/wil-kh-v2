<!--page title start-->

<section class="page-title parallaxie" data-bg-img="images/client/img_7.jpg">
  <div class="container">
    <div class="row">
      <div class="col-lg-6">
        <div class="white-bg p-md-5 p-3 d-inline-block">
        <h1 class="text-theme">Soil Quality <span class="text-black">Testing Packages</span></h1>
        <nav aria-label="breadcrumb" class="page-breadcrumb border-top border-light pt-3 mt-3">
          <ol class="breadcrumb">
            <li class="breadcrumb-item"><a href="<?php echo base_url('/') ?>"><i class="las la-home me-1"></i>Home</a>
            </li>
            <li class="breadcrumb-item">Research
            </li>
            <li class="breadcrumb-item active" aria-current="page">Soil Quality Testing Packages</li>
          </ol>
        </nav>
        </div>
      </div>
    </div>
  </div>
</section>

<!--page title end-->


<!--body content start-->

<div class="page-content">


<!--project start-->

<section class="pt-0">
  <div class="container">
    <div class="row justify-content-center text-center">
      <div class="col-lg-8">
        <div class="section-title"><br><br><br>
          <center><h3>WIL Soil Test Information</h3><br><br>
          
      <table border="2" cellpadding="10" cellspacing="0" style="border-collapse: collapse; border: 1px solid black;">
        <tr>
            <th style="border: 1px solid black;">No</th>
            <th style="border: 1px solid black;">Parameters</th>
            <th style="border: 1px solid black;">Method</th>
            <th style="border: 1px solid black;">Cost per sample</th>
            <th style="border: 1px solid black;">Remarks</th>
        </tr>
        <tr>
            <td style="border: 1px solid black;">1</td>
            <td style="border: 1px solid black;">pH</td>
            <td style="border: 1px solid black;">Electrode</td>
            <td style="border: 1px solid black;">$5.00</td>
            <td style="border: 1px solid black;"></td>
        </tr>
        <tr>
            <td style="border: 1px solid black;">2</td>
            <td style="border: 1px solid black;">Total dissolved solids (TDS)</td>
            <td style="border: 1px solid black;">Electrode</td>
            <td style="border: 1px solid black;">$5.00</td>
            <td style="border: 1px solid black;"></td>
        </tr>
        <tr>
            <td style="border: 1px solid black;">3</td>
            <td style="border: 1px solid black;">Electrical Conductivity</td>
            <td style="border: 1px solid black;">Electrode</td>
            <td style="border: 1px solid black;">$5.00</td>
            <td style="border: 1px solid black;"></td>
        </tr>
        <tr>
            <td style="border: 1px solid black;">4</td>
            <td style="border: 1px solid black;">Salinity (ppt)</td>
            <td style="border: 1px solid black;">Electrode</td>
            <td style="border: 1px solid black;">$7.00</td>
            <td style="border: 1px solid black;"></td>
        </tr>
        <tr>
            <td style="border: 1px solid black;">5</td>
            <td style="border: 1px solid black;">Particles size</td>
            <td style="border: 1px solid black;">Hydrometer</td>
            <td style="border: 1px solid black;">$10.00</td>
            <td style="border: 1px solid black;"></td>
        </tr>
        <tr>
            <td style="border: 1px solid black;">6</td>
            <td style="border: 1px solid black;">Moisture (%)</td>
            <td style="border: 1px solid black;">Gravimetric</td>
            <td style="border: 1px solid black;">$5.00</td>
            <td style="border: 1px solid black;"></td>
        </tr>
        <tr>
            <td style="border: 1px solid black;">7</td>
            <td style="border: 1px solid black;">Nitrogen (Nitrate)</td>
            <td style="border: 1px solid black;">Spectrophotometer</td>
            <td style="border: 1px solid black;">$9.00</td>
            <td style="border: 1px solid black;"></td>
        </tr>
        <tr>
            <td style="border: 1px solid black;">8</td>
            <td style="border: 1px solid black;">Phosphorous (available)</td>
            <td style="border: 1px solid black;">Spectrophotometer</td>
            <td style="border: 1px solid black;">$9.00</td>
            <td style="border: 1px solid black;"></td>
        </tr>
        <tr>
            <td style="border: 1px solid black;">9</td>
            <td style="border: 1px solid black;">Potassium</td>
            <td style="border: 1px solid black;">Spectrophotometer</td>
            <td style="border: 1px solid black;">$10.00</td>
            <td style="border: 1px solid black;"></td>
        </tr>
        <tr>
            <td style="border: 1px solid black;">10</td>
            <td style="border: 1px solid black;">Calcium</td>
            <td style="border: 1px solid black;">Titration</td>
            <td style="border: 1px solid black;">$7.00</td>
            <td style="border: 1px solid black;"></td>
        </tr>
        <tr>
            <td style="border: 1px solid black;">11</td>
            <td style="border: 1px solid black;">Magnesium</td>
            <td style="border: 1px solid black;">Spectrophotometer</td>
            <td style="border: 1px solid black;">$7.00</td>
            <td style="border: 1px solid black;"></td>
        </tr>
        <tr>
            <td style="border: 1px solid black;">12</td>
            <td style="border: 1px solid black;">Oil and Grease</td>
            <td style="border: 1px solid black;">Extraction</td>
            <td style="border: 1px solid black;">$30.00</td>
            <td style="border: 1px solid black;"></td>
        </tr>
        <tr>
            <td style="border: 1px solid black;">13</td>
            <td style="border: 1px solid black;">Arsenic (As)</td>
            <td style="border: 1px solid black;">Digestion, Spectrophotometer</td>
            <td style="border: 1px solid black;">$30.00</td>
            <td style="border: 1px solid black;"></td>
        </tr>
        <tr>
            <td style="border: 1px solid black;">14</td>
            <td style="border: 1px solid black;">Chromium (Cr)</td>
            <td style="border: 1px solid black;">Digestion, Spectrophotometer</td>
            <td style="border: 1px solid black;">$30.00</td>
            <td style="border: 1px solid black;"></td>
        </tr>
        <tr>
            <td style="border: 1px solid black;">15</td>
            <td style="border: 1px solid black;">Iron (Fe)</td>
            <td style="border: 1px solid black;">Digestion, Spectrophotometer</td>
            <td style="border: 1px solid black;">$30.00</td>
            <td style="border: 1px solid black;"></td>
        </tr>
        <tr>
            <td style="border: 1px solid black;">16</td>
            <td style="border: 1px solid black;">Manganese (Mn)</td>
            <td style="border: 1px solid black;">Digestion, Spectrophotometer</td>
            <td style="border: 1px solid black;">$30.00</td>
            <td style="border: 1px solid black;"></td>
        </tr>
        <tr>
            <td style="border: 1px solid black;">17</td>
            <td style="border: 1px solid black;">Cadmium (Cd)</td>
            <td style="border: 1px solid black;">Digestion, Spectrophotometer</td>
            <td style="border: 1px solid black;">$30.00</td>
            <td style="border: 1px solid black;"></td>
        </tr>
        <tr>
            <td style="border: 1px solid black;">18</td>
            <td style="border: 1px solid black;">Lead (Pb)</td>
            <td style="border: 1px solid black;">Digestion, Spectrophotometer</td>
            <td style="border: 1px solid black;">$30.00</td>
            <td style="border: 1px solid black;"></td>
        </tr>
        <tr>
            <td style="border: 1px solid black;">19</td>
            <td style="border: 1px solid black;">Copper (Cu)</td>
            <td style="border: 1px solid black;">Digestion, Spectrophotometer</td>
            <td style="border: 1px solid black;">$30.00</td>
            <td style="border: 1px solid black;"></td>
        </tr>
        <tr>
            <td style="border: 1px solid black;">20</td>
            <td style="border: 1px solid black;">Zinc (Zn)</td>
            <td style="border: 1px solid black;">Digestion, Spectrophotometer</td>
            <td style="border: 1px solid black;">$30.00</td>
            <td style="border: 1px solid black;"></td>
        </tr>
        <tr>
            <td style="border: 1px solid black;">21</td>
            <td style="border: 1px solid black;">Total Nitrogen</td>
            <td style="border: 1px solid black;">Kjeldahl</td>
            <td style="border: 1px solid black;">$15.00</td>
            <td style="border: 1px solid black;"></td>
        </tr>
        <tr>
            <td style="border: 1px solid black;">22</td>
            <td style="border: 1px solid black;">Total Phosphorus</td>
            <td style="border: 1px solid black;">Spectrophotometer</td>
            <td style="border: 1px solid black;">$15.00</td>
            <td style="border: 1px solid black;"></td>
        </tr>
      </table>
      </center>



        </div>
      </div>
    </div>
  </div>
</section>

<!--project end-->

</div>

<!--body content end-->